...
library(ggplot2)
library(lattice)
mydata<- read.csv("C:\\Users\\yinyin\\Desktop\\herbpair\\2. propertyarrangement\\compound finger\\runitself\\used for finger generation\\fitercomher.csv", header =T,sep=',')
...
Pubchem=select(uniquedata2,PubchemFP0:PubchemFP878)
MACCS=select(uniquedata2,MACCSFP3:MACCSFP163)
Sub=select(uniquedata2,SubFP1:SubFP307)
Ext=select(uniquedata2,ExtFP1:ExtFP1022)
...
meridian5=function(datalist,organlist,seedlist,methodlist)
{
  ...
  write.csv(d1,file='C:\\Users\\yinyin\\Desktop\\herbpair\\seed1.csv')
  return(d1)
